...
#include <rslidar_msgs/cameraTrigger.h>

namespace rslidar_driver
{
static const unsigned int POINTS_ONE_CHANNEL_PER_SECOND = 18000;
static const unsigned int BLOCKS_ONE_CHANNEL_PER_PKT = 3;
...
rslidarDriver::rslidarDriver(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  ...
  private_nh.param("model", config_.model, std::string("RS128"));
  double packet_rate;  // packet frequency (Hz)
  std::string model_full_name;

  if (config_.model == "RS128")
  {
    packet_rate = 6000;
    model_full_name = "RS-LiDAR-128";
  }
  else
  {
  ...
  ///////////////////////////for  camera trigger ///////////////////////////
  private_nh.param<int>("camera_num", camera_num_, 1);
  camera_trigger_pub_.resize(camera_num_);
  camera_trigger_topic_.resize(camera_num_);
  camera_trigger_angle_.resize(camera_num_);

  private_nh.param<bool>("trigger_camera_flag", trigger_camera_flag_, false);
  if (trigger_camera_flag_)
  {
    for (std::size_t i = 0; i < camera_num_; ++i)
    {
      std::string topic_name = "camera" + std::to_string(i) + "_trigger_topic";
      std::string topic_default = "/camera" + std::to_string(i) + "_trigger_msg";
      private_nh.param<std::string>(topic_name, camera_trigger_topic_[i], topic_default);
      camera_trigger_pub_[i] = node.advertise<rslidar_msgs::cameraTrigger>(camera_trigger_topic_[i], 10);

      std::string angle_name = "camera" + std::to_string(i) + "_trigger_angle";
      float trigger_angle = 0.0;
      private_nh.param<float>(angle_name, trigger_angle, 0);
      if (trigger_angle < 0 || trigger_angle >= 360)
      {
        std::cerr << "Error: trigger angle is not in [0, 360), set trigger angle to 0 degree" << std::endl;
        trigger_angle = 0.0;
      }
      else
      {
        std::cout << "\nset trigger angle to " << trigger_angle << std::endl << std::endl;
      }
      camera_trigger_angle_[i] = trigger_angle * 100;
    }
  }
  else
  {
    std::cout << "Don't trigger cameras!" << std::endl;
  }
  sequence_id_ = 0;
  /////////////////////////////////////////////////////////////////////////
  ...
  if (dump_file != "")  // have PCAP file?
  {
    ...
    msop_input_.reset(new rslidar_driver::InputPCAP(private_nh, msop_udp_port, npackets, dump_file));
  ...
  std::string output_difop_packets_topic;
  private_nh.param("output_difop_packets_topic", output_difop_packets_topic, std::string("rslidar_packets_difop"));
  difop_output_ = node.advertise<rslidar_msgs::rslidarPacket>(output_difop_packets_topic, 10);
...
{  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
  ...
  rslidar_msgs::cameraTriggerPtr camera_trigger_msg(new rslidar_msgs::cameraTrigger);
  ...
  if (config_.cut_angle >= 0)  // Cut at specific angle feature enabled
  {
    ...
    while (true)
    {
      ...
      int azimuth = 256 * tmp_packet.data[82] + tmp_packet.data[83];
      ...
      if (trigger_camera_flag_)
      {
        for (std::size_t i = 0; i < camera_num_; ++i)
        {
          if (last_azimuth < camera_trigger_angle_[i] && azimuth >= camera_trigger_angle_[i])
          {
            camera_trigger_msg->stamp = tmp_packet.stamp;
            camera_trigger_msg->seq = sequence_id_;
            camera_trigger_pub_[i].publish(camera_trigger_msg);
            break;
          }
        }
      }
  ...
  else  // standard behaviour
  {
    ...
    if (time_synchronization_)
    {
      ...
      double stamp_double =
          mktime(&stm) + 0.001 * (256 * pkt.data[26] + pkt.data[27]) + 0.000001 * (256 * pkt.data[28] + pkt.data[29]);
  ...
  // printf("Publishing a full rslidar scan.\n");
  scan->header.stamp = scan->packets.back().stamp;
  scan->header.frame_id = config_.frame_id;
  scan->header.seq = sequence_id_;
