 ...
 */
InputPCAP::InputPCAP(ros::NodeHandle private_nh, uint16_t port, double npackets, std::string filename, bool read_once,
                     double repeat_delay)
  : Input(private_nh, port), packet_rate_(npackets), filename_(filename)
{
  pcap_ = NULL;
  empty_ = true;
  ...
  private_nh.param("read_hz", read_hz_, 10);
  ...
  packet_rate_ = ros::Rate(npackets * read_hz_);
  ROS_INFO("Read input file at %d hz.", read_hz_);
